...
namespace lir
{
    ...
    inline void dump_value(std::ostream& out, Context&, const binary_op_t& value)
    {
        switch (value)
        {
            case binary_op_t::Multiply:
                out << "Multiply";
                return;
            case binary_op_t::Division:
                out << "Division";
                return;
            case binary_op_t::Modulus:
                out << "Modulus";
                return;
            case binary_op_t::Plus:
                out << "Plus";
                return;
            case binary_op_t::Minus:
                out << "Minus";
                return;
            case binary_op_t::Equals:
                out << "Equals";
                return;
            case binary_op_t::NotEquals:
                out << "NotEquals";
                return;
            case binary_op_t::LessThan:
                out << "LessThan";
                return;
            case binary_op_t::LessThanEqual:
                out << "LessThanEqual";
                return;
            case binary_op_t::GreaterThan:
                out << "GreaterThan";
                return;
            case binary_op_t::GreaterThanEqual:
                out << "GreaterThanEqual";
                return;
            case binary_op_t::AndOp:
                out << "AndOp";
                return;
            case binary_op_t::OrOp:
                out << "OrOp";
                return;
            case binary_op_t::Shl:
                out << "Shl";
                return;
            case binary_op_t::Shr:
                out << "Shr";
                return;
        }
    ...
    )
    {
        std::visit(
            ...
            { dump(out, ctx, alternative, indent); },
            value
        );
    }
    ...
    template <class Context>
    inline void dump(
        std::ostream& out, Context& ctx, const statement& value, int indent
    )
    {
        std::visit(
            [&](const auto& alternative)
    ...
    template <class Context>
    inline void dump(
        std::ostream& out, Context& ctx, const return_statement& value, int indent
    )
    {
        dump_detail::write_indent(out, indent);
        out << "_type: ReturnStatement\n";
        dump_detail::write_indent(out, indent);
        out << "expression:";
        if (!value.expression)
        {
    ...
    template <class Context>
    inline void dump(
        std::ostream& out, Context& ctx, const expression_statement& value, int indent
    )
    {
        dump_detail::write_indent(out, indent);
        out << "_type: ExpressionStatement\n";
        dump_detail::write_indent(out, indent);
        out << "expression:";
        if (!value.expression)
        {
        ...
        out << "is_dropped:";
        out.put(' ');
        dump_value(out, ctx, value.is_dropped);
        out.put('\n');
    }

    template <class Context>
    inline void dump(
        std::ostream& out, Context& ctx, const store_string_statement& value, int indent
    )
    {
        dump_detail::write_indent(out, indent);
        out << "_type: StoreStringStatement\n";
        dump_detail::write_indent(out, indent);
        out << "target:";
        out.put('\n');
        dump(out, ctx, value.target, indent + 4);
        dump_detail::write_indent(out, indent);
        out << "string_table_index:";
        out.put(' ');
        dump_value(out, ctx, value.string_table_index);
        out.put('\n');
        dump_detail::write_indent(out, indent);
        out << "string_size:";
        out.put(' ');
        dump_value(out, ctx, value.string_size);
        out.put('\n');
    }

    template <class Context>
    inline void dump(
        std::ostream& out, Context& ctx, const store_record_statement& value, int indent
    )
    {
        dump_detail::write_indent(out, indent);
        out << "_type: StoreRecordStatement\n";
        dump_detail::write_indent(out, indent);
        out << "target:";
        out.put('\n');
        dump(out, ctx, value.target, indent + 4);
    ...
    template <class Context>
    inline void dump(
        std::ostream& out, Context& ctx, const store_statement& value, int indent
    )
    {
        dump_detail::write_indent(out, indent);
        out << "_type: StoreStatement\n";
        dump_detail::write_indent(out, indent);
        out << "target:";
        out.put('\n');
        dump(out, ctx, value.target, indent + 4);
    ...
    template <class Context>
    inline void dump(
        std::ostream& out, Context& ctx, const while_statement& value, int indent
    )
    {
        dump_detail::write_indent(out, indent);
        out << "_type: WhileStatement\n";
        dump_detail::write_indent(out, indent);
        out << "condition:";
        if (!value.condition)
        {
